...
export class RollController {
    static async rollStatistic(actor, slug, event) {
        const statistic = actor?.getStatistic?.(slug);
        if (!statistic?.roll) {
            console.warn(`${LOG_PREFIX} Statistic check is not available`, { actor: actor?.uuid, slug });
            ui.notifications.warn(`PF2e V2 Player Console: ${slug || "Requested"} statistic is unavailable.`);
            return null;
        }

        return statistic.roll(this.#eventToRollParams(event));
    }
