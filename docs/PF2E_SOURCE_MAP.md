...
The controller accepts only the explicit categories `skill`, `lore`, and `attack`, integer ranks 0–4, and a conservative slug grammar. Regular skills must be own keys of `CONFIG.PF2E.skills`, Lore must resolve to an embedded Lore Item, and attacks must be a raw persistent `custom:true` entry. No DOM-provided property path is accepted. The former `_source.system.skills` existence guard was incorrect: Core declares that source record as `Partial`, builds prepared regular skills by iterating `CONFIG.PF2E.skills`, and treats an absent source rank as 0. On update, `system.skills.<slug>.rank` enters the Foundry Document lifecycle; PF2e `prepareBaseData()` copies the source rank, `prepareDerivedData()` calls `prepareSkills()`, and that method recreates every `Statistic` before writing trace data. The rank listener awaits this lifecycle and performs a final render from current `actor.skills[slug].mod` (whose check exposes the same prepared total at `check.mod`). This avoids the earlier competing hook-only render and any stale Statistic reference. Source, prepared, Statistic, `mod`, and `check.mod` values were the audit checkpoints; no debug logging remains. Perception and saves are an additional summary here; class DCs, armor, standard weapons, synthetic martial entries, and spellcasting are informational. Native tab state and detached-window-local listeners are unchanged.
