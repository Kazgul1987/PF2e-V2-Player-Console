...
| Internal group/slot move | `character/sheet.ts:_onSortItem`; `collection.ts:CharacterFeats.insertFeat` | `actor.feats.insertFeat(item, {groupId,slotId})` | existing same-Actor Feats are updated without copying; collection chooses validity, fallback, slot, or rejection |
