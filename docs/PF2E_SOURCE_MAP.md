...
| Skills | `character/tabs/proficiencies.hbs`; `character/data.ts`; `character/document.ts:prepareBaseData` and `prepareSkills` | `CONFIG.PF2E.skills`; `actor.skills`, prepared `rank`, `mod/value` | yes, owner | `actor.update({"system.skills.<validated-core-slug>.rank": 0..4})` | The regular-skill whitelist is `CONFIG.PF2E.skills`; totals remain PF2e-prepared. Source `system.skills` is `Partial`, so a missing key means rank 0, not an invalid skill. |
...
| Persistent Damage | `item/condition/document.ts`; `actor/base.ts` | `system.persistent`, condition `key`, `decreaseCondition` | condition Item | removal only | Core condition removal | partial; formula/type/DC shown, recovery and counter changes omitted |
...
| Drop handling | `actor/sheet/base.ts#_handleDroppedItem` | `Item.implementation.fromDropData`, Actor condition API, `createEmbeddedDocuments` | embedded Item | owner | supported Item types only | partial; basic Effect/Condition/Affliction drops, no private sheet handler copying or spell-context augmentation |
...
The original character tab is `static/templates/actors/character/tabs/effects.hbs`, its rows are `static/templates/actors/partials/effects.hbs`, and the current Application V2 reference is `src/module/apps/effects-panel.ts`. The actor-sheet drop implementation is protected/internal and therefore is not invoked or copied wholesale. Add Condition UI and the persistent-damage editor are internal applications and remain pending. `EffectPF2e.remainingDuration` is used as prepared output; the module never reads start time, combat round, turn, rule elements, synthetics, or roll options to derive state.
