...
- **M17-11 – External overheal:** At 48/50 HP with only one pending heal of 18, apply it from Chat. Verify PF2e heals to 50, the request disappears, and the section becomes hidden. Repeat after posting a new heal and verify the section appears again.
...
- **M17-14 – Flag-safe and legacy request IDs:** Apply requests for `Actor.XYZ123` and `Scene.ABC.Token.DEF.Actor.XYZ`; verify their new persistent claim keys contain no dots. Seed a claim under the former dotted key shape and verify the corresponding request is treated as applied.
