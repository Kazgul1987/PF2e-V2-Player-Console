...
Application V2 root presentation settings are applied from the single runtime `_onRender()` lifecycle after `super._onRender()`: the console reads the client preferences and writes `data-theme` and `data-density` directly to `this.element`, the final sheet root. Settings changes call the same instance method on each rendered console, so neither a full world reload nor a main-window `document` lookup is involved; detached roots remain isolated.

The Character dashboard is a read-only projection of PF2e prepared Actor data: ability modifiers, movement speeds, prepared languages, armor class, and the prepared held-shield record. The character header owns summary AC; the Character tab has no duplicate AC card and owns detailed shield presentation only when `actor.heldShield` matches `actor.system.attributes.shield.itemId`. Shield name, Hardness, HP, Broken Threshold, and status flags are projected from that prepared record without deriving rules locally. It performs no attribute, speed, shield, language, AC, or DC rule calculation. PF2e's Attribute Builder and Language Selector remain safe partial omissions because Core does not publish them on its runtime API; the edit controls open the official sheet rather than importing private source modules or mutating granted data.

## Persistent Sidebar Architecture

The sidebar is an Application V2 `PART` rendered between the header and native tab navigation. CSS grid places it beside navigation and the active tab panels; it is not a tab panel and the existing `primary` tab wiring remains unchanged. The client-scoped `showSidebar` setting defaults to true. Its change callback rerenders open console applications, while theme and density continue to update sheet-root datasets without global `body` or `html` classes.

`CharacterAdapter` projects prepared PF2e Actor state (HP, AC, held shield, Statistics, ActorInitiative, and the Hero Point resource) into display-only data. The controller exposes only two targeted owner-guarded writes: current HP to `system.attributes.hp.value`, leaving PF2e pre-update handling in charge, and Hero Points through `getResource("hero-points")` / `updateResource("hero-points", value)`, leaving bounds and mythic semantics to PF2e. At narrow detached widths, the left rail becomes a bounded two-column resource region above navigation; disabling it restores a single full-width content column.

Sidebar defensive and status data is presentation-only and comes from PF2e's prepared Actor data. Dying and Wounded use prepared attribute status values, while IWR entries use the prepared Core objects' composite labels; the module performs no local condition or IWR rule evaluation.

## Application V2 sizing and responsive containment

Application sizing is owned by Foundry Application V2: the sheet opts into the native resizable window, retains `position` only for its initial dimensions, and does not implement a parallel resize or drag system. Consequently the Foundry header remains the drag surface and `this.window.resize` remains the sole resize handle. The standard `detachWindow()` lifecycle is also left intact; no main-document query, main-window viewport measurement, or application-owned resize observer is used after the root migrates to the detached document.
