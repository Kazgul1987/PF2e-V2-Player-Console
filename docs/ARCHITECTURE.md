...
`BiographyAdapter` reads the raw PF2e Actor biography, enriches the six Core HTML fields through Foundry's PF2e-aware text pipeline, and applies owner-or-section-visibility filtering before building the part context. `BiographyController` exposes only explicit simple, rich, visibility, and `edicts`/`anathema` field whitelists; list mutations copy raw source arrays and validate indices. The UI has no global form or Save action. Focused simple inputs update on change, while the standard ProseMirror helper owns rich-text save/cancel lifecycle. All Biography listeners use the existing AbortController and the unambiguous application-local `.tab-panel[data-group="primary"][data-tab="biography"]` root, preserving detached and native-tab behavior.
