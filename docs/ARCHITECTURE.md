...
`EffectsController` resolves every ID back to a live PF2e document and exposes only explicit actions. Condition changes delegate to `actor.increaseCondition` / `actor.decreaseCondition`; forced removal still delegates to Core. Affliction stage buttons call the document's `increase` / `decrease`, allowing Core to own linked stage effects. Effect deletion uses its embedded document lifecycle and is omitted for grants. Basic supported drops use Foundry drop-data resolution, then the Actor condition API or embedded Item creation. Rule elements, Active Effects V2, persistent-damage recovery, roll options, synthetics, and prepared values are never interpreted or updated by the module. Listeners remain scoped to the Application element for detached-window safety.
